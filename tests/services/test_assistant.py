...
def assistant_instance(mock_environment_variables):
    with patch("services.assistant.load_tools_from_package") as mock_load_tools, \
         patch("services.assistant.AzureOpenAI") as mock_azure_openai:
        mock_tool = MagicMock()
...
def test_load_tools(assistant_instance):
    ...
    assert assistant_instance.tool_instances[0].get_tool_infos()["function"]["name"] == "mock_tool_function"


def test_extract_tool_schemas(assistant_instance):
    assert len(assistant_instance.tools_schemas) == 1
    assert assistant_instance.tools_schemas[0]["function"]["name"] == "mock_tool_function"
...
def test_call_tool_by_name_success(assistant_instance):
    ...
    result = assistant_instance.call_tool_by_name("mock_tool_function", '{"arg1": "value1"}')
...
def test_call_tool_by_name_tool_not_found(assistant_instance):
    with pytest.raises(ValueError, match="Ferramenta 'non_existent_tool' não encontrada."):
